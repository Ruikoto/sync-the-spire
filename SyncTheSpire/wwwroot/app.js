...
on('PREFLIGHT_FORCE_PUSH', async data => {
    if (data.status === 'success') {
        toast(data.payload?.message || I18n.t('toast.pushed'), 'success');
        sendMessage('GET_STATUS');
...
on('PREFLIGHT_DELETE_LARGE_FILES', async data => {
    if (data.status === 'progress') return;
    if (data.status === 'success') {
        toast(data.payload?.message || I18n.t('modals.preflight.deleteFilesSuccess'), 'success');
        sendMessage('GET_STATUS');
    } else if (data.status === 'conflict') {
        const choice = await showConflictDialog(
            data.payload?.message || I18n.t('modals.conflict.defaultMessage')
        ...
        toast(data.message || I18n.t('modals.preflight.deleteFilesFailed'), 'error');
    }
});
